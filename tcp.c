...
void print_msg(struct tm *curtime, char *msg) {
  dprintf(socketfd, "%02d:%02d:%02d %s\n", curtime->tm_hour, curtime->tm_min,
          curtime->tm_sec, msg);
  if (log_flag)
...
int main(int argc, char **argv) {
  ...
  while (run_flag) {
    ...
    if (pollfd.revents & POLLIN) { // when stdin is readable
      sz = read(pollfd.fd, buf, sizeof(buf));
      if (sz == -1) {
        fprintf(stderr, "Error: read(%s)\n", strerror(errno));
        exit(EXIT_SYSCALL_ERR);
      }
      // Handle cmd
      if (strstr(buf, "SCALE=F") == NULL && strstr(buf, "SCALE=C") == NULL &&
          strstr(buf, "PERIOD=") == NULL && strstr(buf, "STOP") == NULL &&
          strstr(buf, "START") == NULL && strstr(buf, "LOG") == NULL &&
    ...
    memset(buf, 0, 256 * sizeof(buf[0])); // flush buf out
